...
async def parse_title_basics(afp):
...
async def parse_name_basics(afp):
    with Session(engine) as session:
        async for row in aiocsv.AsyncDictReader(afp, delimiter='\t'):
            person = Person(
                nconst=row['nconst'],
...
async def parse_title_principals(afp):
    with Session(engine) as session:
        async for row in aiocsv.AsyncDictReader(afp, delimiter='\t'):
...
async def parse_title_akas(afp):
    with Session(engine) as session:
        async for row in aiocsv.AsyncDictReader(afp, delimiter='\t'):
...
async def parse_title_crew(afp):
    with Session(engine) as session:
        async for row in aiocsv.AsyncDictReader(afp, delimiter='\t'):
            title = session.query(Title).filter_by(tconst=row['tconst']).first()
            if title is None: continue
...
async def parse_title_episode(afp):
    with Session(engine) as session:
        async for row in aiocsv.AsyncDictReader(afp, delimiter='\t'):
            title = session.query(Title).filter_by(tconst=row['tconst']).first()
            if title is None: continue
